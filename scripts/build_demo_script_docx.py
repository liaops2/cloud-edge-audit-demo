#!/usr/bin/env python3
"""Generate DEMO_SCRIPT Word doc using stdlib only."""
...
OUT = ROOT / "DEMO_SCRIPT.docx"

SECTIONS: list[tuple[str, str]] = [
    (
        "上场前（30 秒）",
        """左屏 A：本地直连 · 右屏 B：端云规划+审计
...
过渡：Cloud-Edge Audit 解决「任务是否在独立门禁下算完成」→ 翻第二页""",
    ...
    (
        ...
        """先指 PPT（30 秒）：
① Cloud Planning：拆步骤 + acceptance criteria → plan + criteria 下 Edge
② Local Execution：DingClaw 本地执行 → results + evidence 回 Cloud
③ Cloud Audit：独立审查 → Quality Gate（Pass=完成 / Fail=rework）
...
小结（不翻页）：同 criteria、同 Edge 模型；差在 Plan + Cloud Audit → 翻第三页""",
    ),
    (
...
def para(text: str, *, bold: bool = False, size_half_pt: int = 22) -> str:
    text = escape(text).replace("\n", "</w:t><w:br/><w:t>")
    b = "<w:b/>" if bold else ""
    return (
        f'<w:p><w:pPr><w:spacing w:after="120"/></w:pPr>'
        f'<w:r><w:rPr>{b}<w:sz w:val="{size_half_pt}"/><w:szCs w:val="{size_half_pt}"/>'
...
def build_document_xml() -> str:
    body = [
        para("端云审计 Demo · 现场讲解脚本", bold=True, size_half_pt=32),
        para("第一页 Harness → 第二页 架构+Demo → 第三页 收束 · 约 12–15 分钟", size_half_pt=20),
    ]
    for title, content in SECTIONS:
        body.append(para(title, bold=True, size_half_pt=26))
        for block in content.strip().split("\n\n"):
            body.append(para(block.strip(), size_half_pt=22))
    body.append("</w:body>")
    return (
